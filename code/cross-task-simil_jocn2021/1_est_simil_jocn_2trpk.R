...
# subjs <- subjs[!subjs %in% c("DMCC5820265")]
...
for (glm.i in seq_len(nrow(glminfo))) {
  ...
  betas.i <- readRDS(
    here(
      "out", "glms",
      paste0("betas_dmcc_2trpk_", name.task.i, "_", name.glm.i,  ".RDS")
    )
  )
  ## remove subj with missing data, get target TRs:
  betas.i <- betas.i[, , target.trs.2prk[[glm.i]], ]
...
if (!dir.exists(here("out", "cross-task-simil_jocn2021"))) dir.create(here("out", "cross-task-simil_jocn2021"))
saveRDS(simil, here("out", "cross-task-simil_jocn2021", paste0("corr-biased_unpre_jocn_2trpk.RDS")))
saveRDS(univa, here("out", "cross-task-simil_jocn2021", paste0("mean-biased_unpre_jocn_2trpk.RDS")))
saveRDS(unifo, here("out", "cross-task-simil_jocn2021", paste0("ssq-biased_unpre_jocn_2trpk.RDS")))
