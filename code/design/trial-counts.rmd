...
cuedts.tst <- with(cuedts[cuedts$switch != "", ], table(interaction(subj, run), interaction(cue, switch, trial.type)))
data.frame(
  mean_count = colMeans(cuedts.tst),
  min = apply(cuedts.tst, 2, min),
  max = apply(cuedts.tst, 2, max)
  ) %>%
  kable(caption = "cue \\* trial.type \\* switch")
...
cuedts.tln <- with(cuedts, table(interaction(subj, run), interaction(cue, letter, number)))
data.frame(
  mean_count = colMeans(cuedts.tln),
  min = apply(cuedts.tln, 2, min),
  max = apply(cuedts.tln, 2, max)
  ) %>%
  kable(caption = "cue \\* probetype")


cuedts.tir <- with(cuedts[cuedts$switch != "", ], table(interaction(subj, run), interaction(cue, trial.type, cresp)))
data.frame(
  mean_count = colMeans(cuedts.tir),
  min = apply(cuedts.tir, 2, min),
  max = apply(cuedts.tir, 2, max)
  ) %>%
  kable(caption = "cue \\* trial.type \\* cresp")
...
#     ## get model matrix
#     
#     item.factor <- model.matrix(~ as.factor(unlist(stroop[stroop$subj == subjs[subj.i] & stroop$run == run.i, "item"])) - 1)
...
stern.load01.trial.type <- as.matrix(with(stern, table(interaction(subj, run), load01.trial.type)))

data.frame(
  mean_count = colMeans(stern.load01.trial.type),
  min = apply(stern.load01.trial.type, 2, min),
  max = apply(stern.load01.trial.type, 2, max)
  ) %>%
  kable(caption = "load01 trialtype")

stern.load.trial.type <- as.matrix(with(stern, table(interaction(subj, run), interaction(load, trial.type))))

data.frame(
  mean_count = colMeans(stern.load.trial.type),
  min = apply(stern.load.trial.type, 2, min),
  max = apply(stern.load.trial.type, 2, max)
  ) %>%
  kable(caption = "load01 trialtype")
