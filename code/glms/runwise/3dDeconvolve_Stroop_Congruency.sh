...
for session_i in ${!sessions[@]}; do
	...
	for run_i in ${!runs[@]}; do
		...
		for hemi in ${hemis[@]}; do
			...
			dir_out=${out}${subject}/RESULTS/Stroop/${sessions[$session_i]}_Congruency_EVENTS_censored_shifted_${runs[$run_i]}
			name_img=${img}${subject}/INPUT_DATA/Stroop/${sessions[$session_i]}/lpi_scale_tfMRI_Stroop${sess}${runs[$run_i]}_${encoding_dir[$run_i]}_${hemi}.func.gii
			...
			then
				...
				-stim_times 7 ${dir_stimts}/${subject}_Stroop_${sessions[$session_i]}_buffCon_shifted_run${runs[$run_i]}.txt 'TENTzero(0,16.8,15)' -stim_label 7 buffCon \
