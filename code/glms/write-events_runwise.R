...
subjs <- c(fread(here("in", "ub55_subjects.txt")))
dir.stimts <- here("out", "glms", "stimts")
...
blocks.axcpt  <- axcpt.l %>%  lapply(write.blocks, dir.analysis = dir.stimts, by.run = TRUE)
blocks.axcpt  <- cuedts.l %>% lapply(write.blocks, dir.analysis = dir.stimts, by.run = TRUE)
blocks.stern  <- stern.l %>%  lapply(write.blocks, dir.analysis = dir.stimts, by.run = TRUE)
blocks.stroop <- stroop.l %>% lapply(write.blocks, dir.analysis = dir.stimts, by.run = TRUE)
...
movregs <- split.movregs(to.split, dir.to = dir.stimts)
