...
```{r}
a <- lmer(
  b ~ term*run + (term*run | subj), l$LH_DorsAttn_Post_10[term %in% c("biasInCon", "biasCon")]
  )
summary(a)
...
(tau <- m %*% tau %*% t(m))
(r <- cov2cor(tau)[1, 2])
svd(VarCorr(a)$subj, nv = 0L)$d


tcrossprod(as.matrix(coef(a)$subj), m)
cor(tcrossprod(as.matrix(coef(a)$subj), m))
plot(tcrossprod(as.matrix(coef(a)$subj), m))
...
m0_run <- mclapply(
  l, 
  function(x) lmer(b ~ 0 + term + (0 + term  | subj), x),
  mc.cores = n.cores / 2
  )
