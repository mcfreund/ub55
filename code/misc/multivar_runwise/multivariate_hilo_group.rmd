...
similtype <- c("whitened", "unwhitened")
fname <- c("euclidean-cv-unsta-prewh", "euclidean-cv-unsta-unpre")
...
* Parcels ID'd in =>2 tasks with unwhitened: 
```{r}
dmcc55b.multi.parcels.unwhitened
```

* Parcels ID'd in =>2 tasks with whitened:
```{r}
dmcc55b.multi.parcels.whitened
```
...
This is done with DMCC55B univariate ROIs, for comparability with univariate analysis.
...
parcel.order <- group.target.trs %>%
  ...
  filter(parcel %in% dmcc55b.parcels, similtype == "unwhitened") %>%
...
group.target.trs %>%
  ...
  facet_grid(vars(similtype), scales = "free_y") +
