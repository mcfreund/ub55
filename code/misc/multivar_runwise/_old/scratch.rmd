...
wide <- 
  d %>% 
  .[network %in% c("Cont", "SalVentAttn", "SomMot")] %>%
  select(-network, -hemi) %>%
  dcast(... ~ parcel, value.var = "value")


wide %>%
  
  .[similtype == "euclidean-cv-unsta-unpre" & task == "Axcpt"] %>%
  
  select(-similtype, -task, -subj) %>%
  cor %>%
