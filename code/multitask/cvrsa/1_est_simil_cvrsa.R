...
for (glm.i in seq_len(nrow(glminfo))) {
  ...
  betas.i <- aperm(betas.i, c("vertex", "subj", "reg", "run", "tr"))
  
  betas.i.hi <- rowMeans(betas.i[, , hi[[glm.i]], , ], dims = 3) ## ave across TR, relevant regs
  betas.i.lo <- rowMeans(betas.i[, , lo[[glm.i]], , ], dims = 3) ## ave across TR, relevant regs
  
  contrs[, , name.task.i, ] <- betas.i.hi - betas.i.lo  ## TODO: move to subsequent loop so can standardize.
  
}
rm(betas.i, betas.i.hi, betas.i.lo)
gc()
...
simil <- 
  array(
    ...
    dimnames = list(task = tasks, parcel = parcellation$key, subj = subjs, comparison = c("run11", "run22", "run12", "run21"))
  )
...
for (subj.i in seq_along(subjs))  {
  ...
  for (parcel.i in seq_along(parcellation$key)) {
    ...
    U <- contrs.subj.parcel.i[has.signal.all.conds, , ]
    # U <- U / sqrt(nrow(U))  ## scale by number of vertices
    dim(U) <- c(nrow(U), length(tasks)*2)  ## concatenate runs columnwise
    # colnames(U) <- combo_paste(tasks, c("run1", "run2"))
...
saveRDS(simil, here("out", "multitask", paste0("euclidean-unbiased_unpre.RDS")))
