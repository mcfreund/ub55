---
title: 'multitask pattern similarity: sustained versus rest, cross-validated cosine similarity'
...
* cross-validated cosine similarity on sustained regressor (task versus rest)
...
rsarray <- readRDS(here("out", "multitask", paste0("taskaxis_aggressive1_cosinesim-unbiased_unpre.RDS")))
...
stats.subjs %>%
  ...
  group_by(task, network, subj) %>%
  summarize(value = mean(value)) %>%
  
  ggplot(aes(task, value)) +
  
  stat_summary(fun = "mean", geom = "bar", position = position_dodge(width = 1)) +
  stat_summary(fun.data = "mean_cl_boot", geom = "errorbar", position = position_dodge(width = 1), width = 0, size = 1.5) +
  facet_grid(vars(network)) +
  
  labs(
    y = "group-mean cross-task task-rest discriminability\n(cross-validated cosine simil)", 
    title = "discriminability by network"
    )
...
cat("\n ")
cat("\n### all tasks: fixed-effect analysis (average across tasks)")
cat("\n ")
cat("* parcels with pFDR < 0.05 in all tasks")
...
contrasts.subjs.35 %>%    
  ...
  summarize(
    ...
    statistic = t.test(value)$statistic,
    b = mean(value),
    .groups = "drop_last"
    )
...
contrasts.subjs.35 %>%
  ...
  labs(y = "mean cross-validated cosine similarity")
...
contrast.subjs <- left_join(
  stats.subjs %>% rename(taskrest = value) %>% mutate(subj = as.factor(subj)), contrasts.subjs.hilo %>% rename(hilo = value)
  )
...
contrast.subjs <- full_join(
 ...
 contrast.subjs %>%
  ...
  ggplot(aes(x = z, y = task)) +
  geom_vline(xintercept = 0) +
  geom_boxplot() +
  labs(y = "btw-subj correlation per parcel (z)", title = "subject-level relationship between taskrest and hilo contrast values") +
  facet_grid(vars(network))
