...
d$Axcpt[
  ...
  labs(y = "difference in cross-run correlation", x = "TR post stimtime", title = "Axcpt hilo curves (BX-BY)")
...
d$Axcpt[
 ...
 dcast(... ~ run + term, value.var = "m") %>%
  ...
  labs(y = "difference in cross-run correlation", x = "TR post stimtime", title = "Axcpt hilo curves (BX-BY) DMCC34")
...
s.Axcpt %<>%
  mutate(
    ...
    num.roi = row_number(),
    p.holm = p.adjust(p, "holm")
  )

s.Axcpt %>% filter(p.holm < 0.05) %>% arrange(p.holm) %>% kable
...
s.Axcpt %>%
  mutate(ICC = ifelse(p.holm < 0.05, 1, 0)) %>%
  build_overlay("ICC", template = schaefer) %>%
...
d$Cuedts[, 
  ...
  labs(
    y = "difference in cross-run correlation", 
    x = "TR post stimtime", 
...
s.Cuedts %>% filter(p.holm < 0.05) %>% arrange(p.holm) %>% kable
...
s.Cuedts %>%
  mutate(ICC = ifelse(p.holm < 0.05, ICC, 0)) %>%
  build_overlay("ICC", template = schaefer) %>%
  plot_surface(underlay = hcp, hues = viridis::inferno(500))
...
s.Stern %>% filter(p.holm < 0.05) %>% arrange(p.holm) %>% kable
s.Stern %>% filter(p < 0.05) %>% arrange(p.holm) %>% kable
...
s.Stroop %>% filter(p.holm < 0.05) %>% arrange(p.holm) %>% kable
s.Stroop %>% filter(p < 0.05) %>% arrange(p.holm) %>% kable
