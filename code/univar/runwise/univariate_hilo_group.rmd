...
s.Axcpt %<>% filter(term != "(Intercept)") %>% mutate(p.holm = p.adjust(p.value, "holm"))
...
s.Axcpt %>% filter(p.holm < 0.05 & estimate > 0) %>% arrange(p.holm) %>% kable
s.Axcpt %>% filter(p.holm < 0.05 & estimate > 0) %>% nrow

```


Unthresholded and thresholded t-statistics.
Threshold at holm-corrected p-value (across all parcels) of 0.05.
...
s.Axcpt %>%
  mutate(statistic = ifelse(p.holm < 0.05, statistic, 0)) %>%
  build_overlay("statistic", template = schaefer) %>%
  plot_surface(underlay = hcp, hues = viridis::inferno(500))
...
d$Cuedts[
  ...
  labs(y = "Mean beta", x = "TR post stimtime", title = "Cuedts hilo curves DMCC34 (BX-BY)")
...
s.Cuedts %<>% filter(term != "(Intercept)") %>% mutate(p.holm = p.adjust(p.value, "holm"))
...
s.Cuedts %>% filter(p.holm < 0.05 & estimate > 0) %>% arrange(term, p.holm) %>% kable
s.Cuedts %>% filter(p.holm < 0.05 & estimate > 0) %>% nrow
...
d$Stern[
  ...
  mutate(hilo = (LL5RN + not5RN - LL5NN - not5NN)/2, tr = as.numeric(tr)) %>%
...
Stern <- d$Stern[tr %in% target.trs$Stern & term %in% c("LL5RN", "LL5NN", "not5NN", "not5RN")]
...
s.Stern %<>% filter(term != "(Intercept)") %>% mutate(p.holm = p.adjust(p.value, "holm"))
...
s.Stern %>% filter(p.holm < 0.05 & estimate > 0) %>% arrange(term, p.holm) %>% kable
s.Stern %>% filter(p.holm < 0.05 & estimate > 0) %>% nrow
...
s.Stroop %<>% filter(term != "(Intercept)") %>% mutate(p.holm = p.adjust(p.value, "holm"))
...
s.Stroop %>% filter(p.holm < 0.05 & estimate > 0) %>% arrange(term, p.holm) %>% kable
s.Stroop %>% filter(p.holm < 0.05 & estimate > 0) %>% nrow
...
P-values Holm-corrected across all 400 parcels separately within each task.
...
conj.sumstats <- Reduce(
 intersect, 
   list(
    s.Axcpt  %>% filter(p.holm < 0.05 & estimate > 0) %>% pull(parcel),
    s.Cuedts %>% filter(p.holm < 0.05 & estimate > 0) %>% pull(parcel),
    s.Stern  %>% filter(p.holm < 0.05 & estimate > 0) %>% pull(parcel),
    s.Stroop %>% filter(p.holm < 0.05 & estimate > 0) %>% pull(parcel)
  )
)
...
P-values Holm-corrected across all 400 parcels.
...
m.alltasks <- lapply(alltasks, function(x) lmer(contrast ~ 1 + (1 | subj) + (1 | task), x))
warnings()
s.alltasks <- bind_rows(lapply(m.alltasks, tidy_lmer), .id = "parcel")
s.alltasks %<>% mutate(p.holm = p.adjust(p.value, "holm"))
...
s.alltasks %>% filter(p.holm < 0.05 & estimate > 0) %>% arrange(p.holm) %>% kable
s.alltasks %>% filter(p.holm < 0.05 & estimate > 0) %>% nrow


conj.ranef <- s.alltasks %>% filter(p.holm < 0.05 & estimate > 0) %>% pull(parcel)
