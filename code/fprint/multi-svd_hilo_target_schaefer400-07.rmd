...
d %>%
  ...
  transmute(
    ...
    ndim = ndim,
    estimate.max = estimate[ndim == max(ndim)], 
    # prop.max = estimate / estimate.max
    se.max = se[ndim == max(ndim)],
    pseudo.t = estimate / se,
    pseudo.t.max = estimate.max / se.max,
    prop.max = pseudo.t / pseudo.t.max
...
d %>%
  ...
  filter(type == "cv", task == "Axcpt", parcel %in% parcels.axcpt.midi) %>%
...
d %>%
  ...
  ggplot(aes(ndim, estimate / se)) +
  geom_point() +
  geom_line(aes(group = parcel))
...
d %>%
  ...
  filter(task == "Axcpt", parcel %in% parcels.axcpt.midi, type == "cv") %>%
  
  ggplot(aes(ndim, estimate/se, color = parcel)) +
  geom_line(size = 1.25, alpha = 0.75) +
  
  stat_smooth(color = "black", size = 3) +
  ...
  scale_color_viridis_d() +
  theme(legend.position = "none") +
  labs(y = "MIDI / se", x = "number of dimensions", title = "AXCPT")

d %>%
  
  filter(task == "Stroop", parcel %in% parcels.stroop.midi, type == "cv") %>%
  
  ggplot(aes(ndim, estimate/se, color = parcel)) +
  geom_line(size = 1.25, alpha = 0.75) +
  
  stat_smooth(size = 3, color = "black") +
  ...
  scale_color_viridis_d() +
  theme(legend.position = "none") +
  labs(y = "MIDI / se", x = "number of dimensions", title = "Stroop")
