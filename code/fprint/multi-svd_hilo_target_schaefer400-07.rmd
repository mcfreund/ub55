...
d <- d %>%
  ...
  arrange(transform, task, ndim, num.roi) %>%
...
d %>%
  ...
  filter(parcel %in% intersect(parcels.axcpt.midi, parcels.stroop.midi)) %>%
  
  ggplot(aes(ndim, estimate/se, color = task)) +
...
d %>%
  ...
  filter(get.network(parcel) %in% c("Cont", "SalVentAttn"), task == "Stern") %>%
